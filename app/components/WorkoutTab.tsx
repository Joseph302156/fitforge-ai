...
import {
  getWorkoutPlan, saveWorkoutPlan, deleteWorkoutPlan,
  saveWorkoutLog, getWorkoutLogs,
} from "@/lib/supabase";
...
const HINTS = [
  "Bad knee, avoid high impact", "Only 20 minutes per session",
  "No equipment, bodyweight only", "I travel a lot, hotel workouts",
  "Pregnancy safe workouts", "Night shift worker",
  "Lower back pain", "Weekends only",
];

const CHAT_SUGGESTIONS = [
  "Make Monday easier", "Add more cardio",
  "I only have 20 min per day", "Make it harder overall",
  "Remove all leg exercises", "Add a rest day on Wednesday",
];
      ...
      <div style={{ width:"100%", maxWidth:"384px", background:"white", borderRadius:"24px", overflow:"hidden", maxHeight:"90vh", display:"flex", flexDirection:"column", animation:"slideUp 0.3s ease forwards" }}>
        <div style={{ background:"#1a1a2e", padding:"20px 20px 16px", flexShrink:0 }}>
          <div style={{ display:"flex", justifyContent:"space-between", alignItems:"flex-start", marginBottom:"16px" }}>
            <div style={{ display:"flex", alignItems:"center", gap:"12px" }}>
              ...
              <div>
                <p style={{ color:"white", fontSize:"14px", fontWeight:500, margin:0 }}>{day.name}</p>
                <p style={{ color:"rgba(255,255,255,0.4)", fontSize:"11px", margin:"2px 0 0" }}>{day.duration} · {day.exercises?.length} exercises</p>
              </div>
            </div>
          ...
          <div style={{ textAlign:"center", padding:"12px 0" }}>
            ...
            <p style={{ color:"rgba(255,255,255,0.35)", fontSize:"11px", margin:"4px 0 0" }}>
              {!started ? "Ready to start" : done ? "Workout complete" : "Time elapsed"}
            </p>
          </div>
          {started && !done && (
            <>
        ...
        {!done && (
          <div style={{ flex:1, overflowY:"auto", padding:"16px 20px" }}>
            <p style={{ fontSize:"10px", fontWeight:600, color:"#9ca3af", textTransform:"uppercase", letterSpacing:"0.08em", marginBottom:"12px" }}>Exercises</p>
            <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
              {day.exercises?.map((ex, i) => (
                <button key={i} onClick={() => toggle(i)} disabled={!started || done}
                  style={{ display:"flex", alignItems:"center", gap:"12px", padding:"12px", borderRadius:"12px", border:checked[i] ? "1px solid #bbf7d0" : "1px solid #f3f4f6", background:checked[i] ? "#f0fdf4" : "#f9fafb", cursor:started && !done ? "pointer" : "default", textAlign:"left", opacity:!started ? 0.6 : 1 }}>
                  <div style={{ width:"20px", height:"20px", borderRadius:"50%", border:checked[i] ? "2px solid #22c55e" : "2px solid #d1d5db", background:checked[i] ? "#22c55e" : "transparent", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0 }}>
                  ...
                  <span style={{ fontSize:"12px", color:checked[i] ? "#16a34a" : "#374151", textDecoration:checked[i] ? "line-through" : "none", flex:1 }}>{ex}</span>
                </button>
              ))}
            </div>
          </div>
        ...
        {done && (
          <div style={{ flex:1, display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"32px 20px", textAlign:"center" }}>
            ...
            <h3 style={{ fontSize:"18px", fontWeight:600, color:"#1f2937", margin:"0 0 4px" }}>{allDone ? "Workout complete!" : "Good effort!"}</h3>
            <p style={{ fontSize:"12px", color:"#9ca3af", margin:"0 0 8px" }}>
              {allDone ? `You crushed all ${day.exercises?.length} exercises` : `Completed ${completed} of ${day.exercises?.length}`}
            </p>
    ...
    <div style={{ position:"fixed", inset:0, zIndex:50, display:"flex", alignItems:"flex-end", justifyContent:"center", padding:"16px", background:"rgba(0,0,0,0.35)", backdropFilter:"blur(4px)" }}>
      <div style={{ width:"100%", maxWidth:"384px", background:"white", borderRadius:"20px", overflow:"hidden", animation:"slideUp 0.25s ease forwards" }}>
        <div style={{ background:"#1a1a2e", padding:"16px 20px", display:"flex", justifyContent:"space-between", alignItems:"center" }}>
          <div>
            <p style={{ color:"white", fontSize:"14px", fontWeight:500, margin:0 }}>Edit {day.day}</p>
            <p style={{ color:"rgba(255,255,255,0.4)", fontSize:"11px", margin:"2px 0 0" }}>Customize your workout</p>
          </div>
        ...
        <div style={{ padding:"20px", maxHeight:"70vh", overflowY:"auto" }}>
          ...
          <input value={name} onChange={e => setName(e.target.value)} placeholder="e.g. Upper body strength"
            style={{ width:"100%", background:"#f9fafb", border:"1px solid #e5e7eb", borderRadius:"12px", padding:"10px 12px", fontSize:"13px", color:"#374151", outline:"none", marginBottom:"16px", boxSizing:"border-box" }} />
          <label style={{ display:"block", fontSize:"11px", color:"#9ca3af", fontWeight:500, marginBottom:"6px" }}>Duration</label>
          <input value={duration} onChange={e => setDuration(e.target.value)} placeholder="e.g. 40 min"
            style={{ width:"100%", background:"#f9fafb", border:"1px solid #e5e7eb", borderRadius:"12px", padding:"10px 12px", fontSize:"13px", color:"#374151", outline:"none", marginBottom:"16px", boxSizing:"border-box" }} />
          ...
          <div style={{ display:"flex", flexDirection:"column", gap:"8px", marginBottom:"12px" }}>
            {exercises.map((ex, i) => (
              <div key={i} style={{ display:"flex", alignItems:"center", gap:"8px" }}>
                <span style={{ fontSize:"10px", color:"#d1d5db", width:"16px" }}>{i + 1}</span>
                <input value={ex} onChange={e => { const u = [...exercises]; u[i] = e.target.value; setExercises(u); }}
                  style={{ flex:1, background:"#f9fafb", border:"1px solid #e5e7eb", borderRadius:"8px", padding:"8px 10px", fontSize:"12px", color:"#374151", outline:"none" }} />
          ...
          <div style={{ display:"flex", gap:"8px", marginBottom:"20px" }}>
            <input value={newEx} onChange={e => setNewEx(e.target.value)}
              onKeyDown={e => { if (e.key === "Enter" && newEx.trim()) { setExercises([...exercises, newEx.trim()]); setNewEx(""); } }}
              placeholder="Add exercise... (Enter)"
              style={{ flex:1, background:"#f9fafb", border:"1px dashed #d1d5db", borderRadius:"8px", padding:"8px 10px", fontSize:"12px", color:"#374151", outline:"none" }} />
            <button onClick={() => { if (newEx.trim()) { setExercises([...exercises, newEx.trim()]); setNewEx(""); } }}
              style={{ background:"#4f46e5", color:"white", border:"none", borderRadius:"8px", padding:"8px 12px", fontSize:"12px", cursor:"pointer" }}>+ Add</button>
          </div>
          <button onClick={() => onSave({ ...day, name, duration, exercises })}
            style={{ width:"100%", background:"#4f46e5", color:"white", border:"none", borderRadius:"12px", padding:"12px", fontSize:"13px", fontWeight:500, cursor:"pointer", marginBottom:"8px" }}>Save changes</button>
          <button onClick={onClose}
            style={{ width:"100%", background:"transparent", color:"#9ca3af", border:"1px solid #e5e7eb", borderRadius:"12px", padding:"10px", fontSize:"12px", cursor:"pointer" }}>Cancel</button>
        </div>
      </div>
    </div>
  );
...
function DayCard({ day, onEdit, onStart, isCompleted, isToday, loggedDate }: {
  ...
  return (
    <div onMouseEnter={() => setHovered(true)} onMouseLeave={() => setHovered(false)}
      ...
      style={{ background:"#f9fafb", borderRadius:"12px", border:hovered && isToday ? "1px solid #e5e7eb" : "1px solid #f3f4f6", cursor:isToday ? "pointer" : "default", transition:"border-color 0.15s", opacity:isToday ? 1 : 0.75 }}>
      <div style={{ display:"flex", alignItems:"center", gap:"12px", padding:"12px 12px 8px" }}>
        <div style={{ width:"36px", height:"36px", borderRadius:"8px", background:c.bg, color:c.text, display:"flex", alignItems:"center", justifyContent:"center", fontSize:"10px", fontWeight:500, flexShrink:0 }}>{c.badge}</div>
        <div style={{ flex:1, minWidth:0 }}>
          ...
          {!isToday && <p style={{ fontSize:"10px", color:"#f59e0b", margin:"2px 0 0" }}>available on {day.day}</p>}
        </div>
      ...
      <div style={{ borderLeft:"2px solid #e5e7eb", marginLeft:"16px", paddingLeft:"12px", paddingBottom:"12px", display:"flex", flexDirection:"column", gap:"4px" }}>
        {day.exercises?.map((ex, i) => <p key={i} style={{ fontSize:"10px", color:"#6b7280", margin:0 }}>{ex}</p>)}
      </div>
    </div>
  );
}
...
function ChatBox({ plan, goal, level, onPlanUpdate }: { plan: Plan; goal: string; level: string; onPlanUpdate: (p: Plan) => void }) {
  ...
  async function send(text?: string) {
    ...
    const next = [...msgs, { role:"user", text:t, updated:false }];
    setMsgs(next);
    const summary = plan.days.map(d => d.type === "rest" ? `${d.day}: Rest` : `${d.day}: ${d.name} (${d.duration}) — ${d.exercises?.join(", ")}`).join("\n");
    try {
      const res = await fetch("/api/chat", { method:"POST", headers:{ "Content-Type":"application/json" }, body:JSON.stringify({ goal, level, planSummary:summary, messages:next.map(m => ({ role:m.role === "assistant" ? "assistant" : "user", content:m.text })) }) });
      const data = await res.json();
      if (data.updatedPlan) { onPlanUpdate(data.updatedPlan); setMsgs(p => [...p, { role:"assistant", text:data.message || "Plan updated!", updated:true }]); }
      else setMsgs(p => [...p, { role:"assistant", text:data.message || "Let me know if you need changes!", updated:false }]);
  ...
  return (
    <div style={{ marginTop:"16px" }}>
      <button onClick={() => setOpen(o => !o)}
        style={{ width:"100%", display:"flex", alignItems:"center", justifyContent:"space-between", padding:"12px 16px", borderRadius:"16px", border:`1px solid ${open ? "#1a1a2e" : "#e5e7eb"}`, background:open ? "#1a1a2e" : "white", cursor:"pointer" }}>
        <div style={{ display:"flex", alignItems:"center", gap:"8px" }}>
          <div style={{ width:"20px", height:"20px", borderRadius:"50%", background:"#eef2ff", display:"flex", alignItems:"center", justifyContent:"center" }}>
            <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#4f46e5" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
          </div>
          <span style={{ fontSize:"12px", fontWeight:500, color:open ? "white" : "#374151" }}>Ask your AI trainer</span>
          {msgs.length > 1 && <span style={{ fontSize:"10px", padding:"2px 6px", borderRadius:"99px", background:open ? "rgba(255,255,255,0.15)" : "#eef2ff", color:open ? "white" : "#4f46e5", fontWeight:500 }}>{msgs.length - 1}</span>}
        </div>
      ...
      {open && (
        <div style={{ marginTop:"8px", border:"1px solid #f3f4f6", borderRadius:"16px", overflow:"hidden", background:"white" }}>
          <div style={{ display:"flex", flexDirection:"column", gap:"12px", padding:"16px", maxHeight:"260px", overflowY:"auto" }}>
            {msgs.map((m, i) => (
              <div key={i} style={{ display:"flex", justifyContent:m.role === "user" ? "flex-end" : "flex-start" }}>
                {m.role === "assistant" && (
                  <div style={{ width:"24px", height:"24px", borderRadius:"50%", background:"#eef2ff", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, marginRight:"8px", marginTop:"2px" }}>
                    <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#4f46e5" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
                  </div>
                )}
                <div style={{ display:"flex", flexDirection:"column", gap:"4px", maxWidth:"80%" }}>
                  <div style={{ fontSize:"12px", lineHeight:1.6, padding:"8px 12px", borderRadius:"16px", background:m.role === "user" ? "#1a1a2e" : "#f3f4f6", color:m.role === "user" ? "white" : "#374151", borderBottomRightRadius:m.role === "user" ? "4px" : "16px", borderBottomLeftRadius:m.role === "assistant" ? "4px" : "16px" }}>{m.text}</div>
                  {m.updated && <span style={{ fontSize:"10px", color:"#16a34a", fontWeight:500, paddingLeft:"4px" }}>✓ Plan updated above</span>}
                </div>
              </div>
            ))}
            {loading && (
              <div style={{ display:"flex", justifyContent:"flex-start" }}>
                <div style={{ width:"24px", height:"24px", borderRadius:"50%", background:"#eef2ff", display:"flex", alignItems:"center", justifyContent:"center", flexShrink:0, marginRight:"8px" }}>
                  <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#4f46e5" strokeWidth="2.5" strokeLinecap="round" strokeLinejoin="round"><path d="M21 15a2 2 0 0 1-2 2H7l-4 4V5a2 2 0 0 1 2-2h14a2 2 0 0 1 2 2z"/></svg>
                </div>
          ...
          {msgs.length === 1 && (
            <div style={{ padding:"0 16px 12px", display:"flex", gap:"6px", flexWrap:"wrap" }}>
              {CHAT_SUGGESTIONS.map(s => (
                <button key={s} onClick={() => send(s)} style={{ fontSize:"10px", padding:"4px 10px", borderRadius:"8px", border:"1px solid #e5e7eb", background:"#f9fafb", color:"#6b7280", cursor:"pointer" }}>{s}</button>
              ))}
            </div>
          )}
          <div style={{ display:"flex", alignItems:"center", gap:"8px", padding:"12px", borderTop:"1px solid #f3f4f6" }}>
            <input value={input} onChange={e => setInput(e.target.value)} onKeyDown={e => e.key === "Enter" && !e.shiftKey && send()}
              placeholder="Ask anything about your plan..." disabled={loading}
              style={{ flex:1, fontSize:"12px", background:"#f9fafb", border:"1px solid #e5e7eb", borderRadius:"12px", padding:"8px 12px", color:"#374151", outline:"none" }} />
            <button onClick={() => send()} disabled={!input.trim() || loading}
              style={{ width:"32px", height:"32px", borderRadius:"10px", border:"none", background:input.trim() && !loading ? "#4f46e5" : "#f3f4f6", display:"flex", alignItems:"center", justifyContent:"center", cursor:"pointer", flexShrink:0 }}>
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke={input.trim() && !loading ? "white" : "#9ca3af"} strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><line x1="22" y1="2" x2="11" y2="13"/><polygon points="22 2 15 22 11 13 2 9 22 2"/></svg>
            </button>
          </div>
        </div>
      )}
...
export default function WorkoutTab({ onWorkoutComplete }: { onWorkoutComplete: (n: string, d: string, c: number, s: number) => void }) {
  ...
  async function updatePlan(p: Plan) {
    setPlan(p);
    await saveWorkoutPlan(userId, weekKey, p);
  }
  ...
  async function startOver() {
    await deleteWorkoutPlan(userId, weekKey);
    setPlan(null); setError(""); setPrompt("");
  }
    ...
    return (
      <>
        <div style={{ background:"#1a1a2e", padding:"20px" }}>
          ...
          <p style={{ color:"rgba(255,255,255,0.4)", fontSize:"12px", margin:"4px 0 0" }}>Loading your plan...</p>
        </div>
      ...
      <div style={{ padding:"20px" }}>
          ...
          <>
            ...
            <div style={{ display:"flex", gap:"8px", flexWrap:"wrap", marginBottom:"16px" }}>
              {["Lose weight","Build muscle","Stay fit"].map(g => (
                <button key={g} onClick={() => setGoal(g)} style={{ fontSize:"12px", padding:"6px 14px", borderRadius:"99px", border:`1px solid ${goal === g ? "#1a1a2e" : "#e5e7eb"}`, background:goal === g ? "#1a1a2e" : "transparent", color:goal === g ? "white" : "#6b7280", cursor:"pointer" }}>{g}</button>
              ))}
            </div>
            <p style={{ fontSize:"12px", color:"#9ca3af", fontWeight:500, marginBottom:"8px" }}>Fitness level</p>
            <div style={{ display:"flex", gap:"8px", flexWrap:"wrap", marginBottom:"20px" }}>
              {["Beginner","Intermediate","Advanced"].map(l => (
                <button key={l} onClick={() => setLevel(l)} style={{ fontSize:"12px", padding:"6px 14px", borderRadius:"99px", border:`1px solid ${level === l ? "#1a1a2e" : "#e5e7eb"}`, background:level === l ? "#1a1a2e" : "transparent", color:level === l ? "white" : "#6b7280", cursor:"pointer" }}>{l}</button>
              ))}
            </div>
            <hr style={{ border:"none", borderTop:"1px solid #f3f4f6", marginBottom:"20px" }} />
            <div style={{ display:"flex", alignItems:"center", gap:"8px", marginBottom:"8px" }}>
              <p style={{ fontSize:"12px", fontWeight:500, color:"#374151", margin:0 }}>Tell the AI your situation</p>
              <span style={{ fontSize:"10px", background:"#eef2ff", color:"#4f46e5", padding:"2px 8px", borderRadius:"99px", fontWeight:500 }}>AI</span>
            </div>
            <textarea value={prompt} onChange={e => setPrompt(e.target.value)} rows={4}
              placeholder="e.g. I have a bad knee so no running. I'm free Mon, Wed, Fri. I only have dumbbells at home..."
              style={{ width:"100%", fontSize:"12px", border:"1px solid #e5e7eb", borderRadius:"12px", padding:"12px", color:"#374151", background:"#f9fafb", resize:"none", outline:"none", lineHeight:1.6, boxSizing:"border-box" }} />
            <div style={{ display:"flex", gap:"6px", flexWrap:"wrap", marginTop:"8px", marginBottom:"20px" }}>
              {HINTS.map(h => (
                <button key={h} onClick={() => setPrompt(p => p.trim() ? p.trimEnd() + ". " + h : h)}
                  style={{ fontSize:"10px", padding:"4px 10px", borderRadius:"6px", background:"#f9fafb", border:"1px solid #e5e7eb", color:"#9ca3af", cursor:"pointer" }}>{h}</button>
              ))}
            </div>
            <button onClick={generate} style={{ width:"100%", background:"#4f46e5", color:"white", border:"none", borderRadius:"12px", padding:"12px", fontSize:"14px", fontWeight:500, cursor:"pointer" }}>
        ...
        {loading && (
          <div style={{ display:"flex", flexDirection:"column", alignItems:"center", justifyContent:"center", padding:"64px 0", gap:"16px" }}>
            <div style={{ width:"32px", height:"32px", borderRadius:"50%", border:"2px solid #e5e7eb", borderTopColor:"#6366f1", animation:"spin 0.8s linear infinite" }} />
            <p style={{ fontSize:"12px", color:"#9ca3af" }}>Building your personalized plan...</p>
          </div>
        )}
          ...
          <>
            ...
            {plan.tip && (
              <div style={{ background:"#eef2ff", borderRadius:"12px", padding:"12px", marginBottom:"16px" }}>
                ...
                <p style={{ fontSize:"12px", color:"#3730a3", lineHeight:1.6, margin:0 }}>{plan.tip}</p>
              </div>
            )}
            <div style={{ display:"flex", flexDirection:"column", gap:"8px" }}>
              {plan.days.map((day, i) => {
