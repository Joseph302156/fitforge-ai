    ...
    <div style={{position:"fixed",inset:0,zIndex:50,display:"flex",alignItems:"flex-end",justifyContent:"center",padding:"16px",background:"rgba(0,0,0,0.35)",backdropFilter:"blur(4px)"}}>
      <div style={{width:"100%",maxWidth:"384px",background:"white",borderRadius:"20px",overflow:"hidden",animation:"slideUp 0.25s ease forwards"}}>
        <div style={{background:"#1a1a2e",padding:"16px 20px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div><p style={{color:"white",fontSize:"14px",fontWeight:500,margin:0}}>Edit {cfg.label.toLowerCase()}</p><p style={{color:"rgba(255,255,255,0.4)",fontSize:"11px",margin:"2px 0 0"}}>Set your daily target</p></div>
        ...
        <div style={{padding:"20px"}}>
          ...
          <div style={{display:"flex",gap:"10px",alignItems:"center",marginBottom:"20px"}}>
            <input type="number" value={val} onChange={e=>setVal(e.target.value)} style={{flex:1,background:cfg.bg,border:`1px solid ${cfg.color}40`,borderRadius:"12px",padding:"12px 14px",fontSize:"20px",fontWeight:500,color:cfg.color,outline:"none",textAlign:"center"}}/>
...
function AddMealModal({dateStr,onAdd,onClose}:{dateStr:string;onAdd:(m:Meal)=>void;onClose:()=>void}) {
  ...
  return (
    <div style={{position:"fixed",inset:0,zIndex:50,display:"flex",alignItems:"flex-end",justifyContent:"center",padding:"16px",background:"rgba(0,0,0,0.35)",backdropFilter:"blur(4px)"}}>
      <div style={{width:"100%",maxWidth:"384px",background:"white",borderRadius:"20px",overflow:"hidden",animation:"slideUp 0.25s ease forwards"}}>
        <div style={{background:"#1a1a2e",padding:"16px 20px",display:"flex",justifyContent:"space-between",alignItems:"center"}}>
          <div><p style={{color:"white",fontSize:"14px",fontWeight:500,margin:0}}>Log a meal</p><p style={{color:"rgba(255,255,255,0.4)",fontSize:"11px",margin:"2px 0 0"}}>{dateLabel}</p></div>
          <button onClick={onClose} style={{background:"none",border:"none",color:"rgba(255,255,255,0.4)",fontSize:"18px",cursor:"pointer"}}>✕</button>
        </div>
...
function AIMealCalculator({dateStr,onLog,onClose}:{dateStr:string;onLog:(m:Meal)=>void;onClose:()=>void}) {
  ...
  async function send(text?:string) {
    ...
    try {
      const res=await fetch("/api/meal-calc",{method:"POST",headers:{"Content-Type":"application/json"},body:JSON.stringify({messages:newMsgs.map(m=>({role:m.role,content:m.content}))})});
      const data=await res.json();
      if(data.type==="estimate"&&data.meal){const meal:Meal={id:Date.now().toString(),...data.meal};setMsgs(p=>[...p,{role:"assistant",content:data.message,estimate:meal}]);}
      else setMsgs(p=>[...p,{role:"assistant",content:data.message}]);
      ...
      <div style={{width:"100%",maxWidth:"384px",background:"white",borderRadius:"24px",overflow:"hidden",maxHeight:"88vh",display:"flex",flexDirection:"column",animation:"slideUp 0.3s ease forwards"}}>
        <div style={{background:"#1a1a2e",padding:"16px 20px",flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start"}}>
            <div><p style={{color:"white",fontSize:"14px",fontWeight:500,margin:0}}>AI meal calculator</p><p style={{color:"rgba(255,255,255,0.4)",fontSize:"11px",margin:"2px 0 0"}}>{dateLabel}</p></div>
        ...
        <div style={{flex:1,overflowY:"auto",padding:"16px",display:"flex",flexDirection:"column",gap:"12px"}}>
          {msgs.map((m,i)=>(
            <div key={i} style={{display:"flex",flexDirection:"column",gap:"8px"}}>
              <div style={{display:"flex",justifyContent:m.role==="user"?"flex-end":"flex-start"}}>
                {m.role==="assistant"&&<div style={{width:"26px",height:"26px",borderRadius:"50%",background:"#fff7ed",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginRight:"8px",marginTop:"2px"}}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#ea580c" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8h1a4 4 0 0 1 0 8h-1"/><path d="M2 8h16v9a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V8z"/><line x1="6" y1="1" x2="6" y2="4"/><line x1="10" y1="1" x2="10" y2="4"/><line x1="14" y1="1" x2="14" y2="4"/></svg></div>}
                <div style={{maxWidth:"78%",fontSize:"12px",lineHeight:1.6,padding:"9px 12px",borderRadius:"16px",background:m.role==="user"?"#1a1a2e":"#f3f4f6",color:m.role==="user"?"white":"#374151",borderBottomRightRadius:m.role==="user"?"4px":"16px",borderBottomLeftRadius:m.role==="assistant"?"4px":"16px"}}>{m.content}</div>
              </div>
              {m.estimate&&(
                <div style={{marginLeft:"34px",background:"white",border:"1px solid #e5e7eb",borderRadius:"14px",overflow:"hidden"}}>
                  <div style={{background:"#f9fafb",padding:"10px 14px",borderBottom:"1px solid #f3f4f6"}}><p style={{fontSize:"12px",fontWeight:500,color:"#1f2937",margin:0}}>{m.estimate.name}</p></div>
                  <div style={{padding:"10px 14px"}}>
                    <div style={{display:"flex",alignItems:"center",justifyContent:"space-between",marginBottom:"8px",background:"#eef2ff",borderRadius:"8px",padding:"7px 10px"}}><span style={{fontSize:"11px",fontWeight:500,color:"#4338ca"}}>Calories</span><span style={{fontSize:"14px",fontWeight:500,color:"#1e1b4b"}}>{m.estimate.calories} kcal</span></div>
                    <div style={{display:"grid",gridTemplateColumns:"1fr 1fr 1fr",gap:"6px",marginBottom:"10px"}}>
                      <div style={{background:"#f0fdf4",borderRadius:"8px",padding:"6px 8px",textAlign:"center"}}><div style={{fontSize:"9px",color:"#15803d",fontWeight:500}}>Protein</div><div style={{fontSize:"14px",fontWeight:500,color:"#14532d"}}>{m.estimate.protein}g</div></div>
                      <div style={{background:"#fff7ed",borderRadius:"8px",padding:"6px 8px",textAlign:"center"}}><div style={{fontSize:"9px",color:"#c2410c",fontWeight:500}}>Carbs</div><div style={{fontSize:"14px",fontWeight:500,color:"#7c2d12"}}>{m.estimate.carbs}g</div></div>
                      <div style={{background:"#faf5ff",borderRadius:"8px",padding:"6px 8px",textAlign:"center"}}><div style={{fontSize:"9px",color:"#7e22ce",fontWeight:500}}>Fat</div><div style={{fontSize:"14px",fontWeight:500,color:"#581c87"}}>{m.estimate.fat}g</div></div>
                    </div>
                    <div style={{display:"flex",gap:"6px"}}>
                      <button onClick={()=>{onLog(m.estimate!);onClose();}} style={{flex:1,background:"#1a1a2e",color:"white",border:"none",borderRadius:"10px",padding:"9px",fontSize:"12px",fontWeight:500,cursor:"pointer"}}>Log this meal</button>
                      <button onClick={()=>send("Can you adjust the estimate?")} style={{flex:1,background:"transparent",color:"#6b7280",border:"1px solid #e5e7eb",borderRadius:"10px",padding:"9px",fontSize:"12px",cursor:"pointer"}}>Adjust</button>
                    </div>
                  </div>
                </div>
              )}
          ...
          {loading&&(
            <div style={{display:"flex",justifyContent:"flex-start"}}>
              <div style={{width:"26px",height:"26px",borderRadius:"50%",background:"#fff7ed",display:"flex",alignItems:"center",justifyContent:"center",flexShrink:0,marginRight:"8px"}}><svg width="12" height="12" viewBox="0 0 24 24" fill="none" stroke="#ea580c" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8h1a4 4 0 0 1 0 8h-1"/><path d="M2 8h16v9a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V8z"/><line x1="6" y1="1" x2="6" y2="4"/><line x1="10" y1="1" x2="10" y2="4"/><line x1="14" y1="1" x2="14" y2="4"/></svg></div>
          ...
          {msgs.length===1&&!loading&&(
            <div style={{display:"flex",flexDirection:"column",gap:"6px",marginTop:"4px"}}>
              ...
              {suggestions.map(s=><button key={s} onClick={()=>send(s)} style={{textAlign:"left",background:"#f9fafb",border:"1px solid #f3f4f6",borderRadius:"10px",padding:"8px 12px",fontSize:"11px",color:"#6b7280",cursor:"pointer"}}>{s}</button>)}
            </div>
          )}
          <div ref={bottomRef}/>
        </div>
        <div style={{padding:"12px 14px",borderTop:"1px solid #f3f4f6",flexShrink:0,display:"flex",gap:"8px",alignItems:"center"}}>
          <input value={input} onChange={e=>setInput(e.target.value)} onKeyDown={e=>e.key==="Enter"&&!e.shiftKey&&send()} placeholder="Describe what you ate..." disabled={loading} style={{flex:1,fontSize:"12px",background:"#f9fafb",border:"1px solid #e5e7eb",borderRadius:"12px",padding:"9px 12px",color:"#374151",outline:"none"}}/>
          <button onClick={()=>send()} disabled={!input.trim()||loading} style={{width:"34px",height:"34px",borderRadius:"10px",border:"none",background:input.trim()&&!loading?"#4f46e5":"#f3f4f6",display:"flex",alignItems:"center",justifyContent:"center",cursor:"pointer",flexShrink:0}}>
...
export default function NutritionTab() {
  ...
  useEffect(()=>{
    if(!userId) return;
    async function load() {
      const [logs, savedGoals] = await Promise.all([
      ...
      setData(typedLogs);
      if (savedGoals) setGoals(savedGoals as Goals);
      setLoaded(true);
    }
    load();
  },[userId]);
  ...
  async function handleAddMeal(meal: Meal) {
    ...
    const newMeals = [...existing, meal];
    const newData = {...data, [selectedDate]:{meals:newMeals}};
    setData(newData);
    await saveNutritionLog(userId, selectedDate, newMeals);
    setShowAddMeal(false);
  }

  async function handleLogAIMeal(meal: Meal) {
    const existing = data[selectedDate]?.meals || [];
    const newMeals = [...existing, {...meal, id:Date.now().toString()}];
    const newData = {...data, [selectedDate]:{meals:newMeals}};
    setData(newData);
    await saveNutritionLog(userId, selectedDate, newMeals);
  }
  ...
  async function handleDeleteMeal(id: string) {
    const existing = data[selectedDate]?.meals || [];
    const newMeals = existing.filter(m=>m.id!==id);
    const newData = {...data, [selectedDate]:{meals:newMeals}};
    setData(newData);
    await saveNutritionLog(userId, selectedDate, newMeals);
  }

  async function handleSaveGoal(field: keyof Goals, val: number) {
    const newGoals = {...goals, [field]:val};
    setGoals(newGoals);
    await saveNutritionGoals(userId, newGoals);
    setEditingGoal(null);
  }
    ...
    return (
      <>
        <div style={{background:"#1a1a2e",padding:"20px"}}>
          ...
          <p style={{color:"rgba(255,255,255,0.4)",fontSize:"12px",margin:"4px 0 0"}}>Loading your nutrition data...</p>
        </div>
  ...
  return (
    <>
      ...
      <div style={{padding:"14px"}}>
        ...
        <button onClick={()=>setShowAddMeal(true)} style={{width:"100%",marginTop:"12px",padding:"11px",borderRadius:"12px",border:"1.5px dashed #e5e7eb",background:"transparent",fontSize:"12px",color:"#9ca3af",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:"8px"}}>
        ...
        <button onClick={()=>setShowAICalc(true)} style={{width:"100%",marginTop:"8px",padding:"11px",borderRadius:"12px",border:"1.5px solid #fed7aa",background:"#fff7ed",fontSize:"12px",color:"#c2410c",cursor:"pointer",display:"flex",alignItems:"center",justifyContent:"center",gap:"8px"}}>
          <svg width="14" height="14" viewBox="0 0 24 24" fill="none" stroke="#ea580c" strokeWidth="2" strokeLinecap="round" strokeLinejoin="round"><path d="M18 8h1a4 4 0 0 1 0 8h-1"/><path d="M2 8h16v9a4 4 0 0 1-4 4H6a4 4 0 0 1-4-4V8z"/><line x1="6" y1="1" x2="6" y2="4"/><line x1="10" y1="1" x2="10" y2="4"/><line x1="14" y1="1" x2="14" y2="4"/></svg>
